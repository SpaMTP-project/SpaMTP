...
visualisePathways = function(SpaMTP,
  ...
  for(z in seq_len(nrow(merged_pathways))){
    ...
    if ("mz-" %in% mzs){
      ...
      mat_ind = which(rownames(SpaMTP[[assay]]) %in% mzs)
...
addGesecaScores <- function(pathways,
  ...
  for (i in seq_along(pathways)) {
    ...
    scoreMetadata <- data.frame(score)
    names(scoreMetadata) <- paste0(prefix, names(pathways)[i])
    rownames(scoreMetadata) <- colnames(res)
    res <- .setCellMetadata(res, scoreMetadata)
  }
